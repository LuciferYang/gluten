  ...
  testGluten("fallbackSummary with data write command") {
    withAQEEnabledAndDisabled {
      withTable("tmp") {
        ...
        // spark33 expected one Gluten node here. Since Spark 3.4 the CTAS is executed as an
        // ExecutedCommandExec, which collectFallbackNodes walks past without counting
        // anything, so the summary reports neither a Gluten node nor a fallback node.
        assert(df.fallbackSummary().numGlutenNodes == 0, df.fallbackSummary())
        assert(df.fallbackSummary().numFallbackNodes == 0, df.fallbackSummary())
      }
    }
