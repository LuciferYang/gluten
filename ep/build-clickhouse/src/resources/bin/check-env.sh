...
function check_spark_version() {
  ...
  if [[ "${SPARK_MAJOR_MINOR_VERSION}" != "3.4" ]] && [[ "${SPARK_MAJOR_MINOR_VERSION}" != "3.5" ]] && [[ "${SPARK_MAJOR_MINOR_VERSION}" != "4.0" ]]; then
    echo "[ERROR] SPARK_VERSION ${SPARK_VERSION} which defined in $SPARK_HOME/RELEASE, is not supported. Please use spark 3.4, 3.5, or 4.0."
    exit 1
  fi
